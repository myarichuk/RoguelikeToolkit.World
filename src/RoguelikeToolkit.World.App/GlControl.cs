...
namespace RoguelikeToolkit.World.App
{
    public unsafe class GlControl : OpenGlControlBase
    {
        ...
        protected override void OnOpenGlRender(GlInterface gl, int fb)
        {
            ...
            Span<float> modelX = stackalloc float[16];
            CreateRotationX(Pitch, modelX);
            Span<float> modelY = stackalloc float[16];
        ...
        private void CreatePerspective(float fov, float aspect, float zNear, float zFar, Span<float> result)
        {
            result.Clear();
        ...
        private void CreateTranslation(float x, float y, float z, Span<float> result)
        {
        ...
        private void CreateRotationX(float angleDegrees, Span<float> result)
        {
        ...
        private void CreateRotationY(float angleDegrees, Span<float> result)
        {
            float angle = angleDegrees * (float)Math.PI / 180.0f;
            float c = (float)Math.Cos(angle);
            float s = (float)Math.Sin(angle);
        ...
        private void MultiplyMatrix(ReadOnlySpan<float> a, ReadOnlySpan<float> b, Span<float> result)
        {
            for (int c = 0; c < 4; c++)
            {
                for (int r = 0; r < 4; r++)
