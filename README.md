...
- `WorldDataStore` also caches per-size tile unit vectors (`GetTileVectors()`), so repeated generation runs avoid rebuilding spherical tile positions and repeated trig conversions in hot paths.
